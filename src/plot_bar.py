...
def plot(exp_data, iso_data, percent):
    ...
    if percent == True:
        ...
        ax.barh(iso_10ft['state_name'], iso_10ft['U7B001'], color='#a5afc3', label='isolated at 10ft SLR')
        ax.barh(iso_6ft['state_name'], iso_6ft['U7B001'], color='#627397', label='isolated at 6ft SLR')
        ax.barh(iso_3ft['state_name'], iso_3ft['U7B001'], color='#1f386b', label='isolated at 3ft SLR')
        ax.plot(exp_10ft['U7B001'], exp_10ft['state_name'], color='darkred', label='exposed at 10ft SLR')
        ax.plot(exp_6ft['U7B001'], exp_6ft['state_name'], color='firebrick', label='exposed at 6ft SLR')
        ax.plot(exp_3ft['U7B001'], exp_3ft['state_name'], color='lightcoral', label='exposed at 3ft SLR')
        ax.set_xlabel('Number of people')
        ax.legend(loc='lower right')
        ax.set_yticklabels(exp_10ft['state_name'], rotation=0)
        plt.tight_layout()
        plt.savefig('src/figs/bar_population.jpg')
