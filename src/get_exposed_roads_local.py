...
for row in tqdm(data):  # reversed(data):
    ...
    # get bounding box
    layer_index = [i for i, s in enumerate(layers) if 'slr_10ft' in s][0]
    gdf = gpd.read_file(fng, driver='OpenFileGDB', layer=layers[layer_index])
    bbox =  gdf.total_bounds
    xmin,ymin,xmax,ymax = float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3])
    # get road network
    logger.error('Downloading edges for {}'.format(name))
    ...
    # drop unused columns
    road_gdf = road_gdf.drop(columns=['osmid', 'lanes', 'name', 'highway', 'oneway', 'length', 'ref', 'maxspeed', 'junction', 'access', 'tunnel', 'width', 'key', 'bridge'])
    # rename columns
    road_gdf = road_gdf.rename(columns={"u": "to_osmid", "v": "from_osmid"})
    # get length
    proj_roads = road_gdf.to_crs(3395)
    road_gdf['length'] = list(proj_roads.length.round(1))
    logger.error('{} road network complete'.format(name))
    for layer in layers:
        ...
        gdf = gdf[['geometry','rise','inundation', 'region', 'layer']]
        # dissolve
        gdf = gdf.dissolve(by='rise')
        # overlap
        logger.info('Calculating road network exposure')

        exposed_roads = gpd.clip(road_gdf, gdf)
        # make results df
        df = pd.DataFrame(exposed_roads[['from_osmid', 'to_osmid', 'length', 'rise', 'inundation', 'region']])
        # # save
        df.to_sql('exposed_roads_local', engine, if_exists='append', index=False)
        #gdf.to_postgis('slr_raw', engine, if_exists='append',
            #dtype={'geometry': Geometry('POLYGON', srid=crs)})
        # code.interact(local=locals())
        con.commit()
        logger.info('{} exposed roads appended to SQL'.format(len(exposed_roads)))
