    ...
    for rise in tqdm(rises):
        for inundation in inundations:
            ...
            queries = ["SET work_mem = '500MB'", "SET max_parallel_workers = '8'", "SET max_parallel_workers_per_gather = '8'",
                       """CREATE TABLE IF NOT EXISTS exposed_snaps20
                    (id_orig VARCHAR(255),
                    geoid_county VARCHAR(255),
                    rise INT,
                    inundation VARCHAR(255));""",

                       """INSERT INTO exposed_snaps20 (id_orig, geoid_county, rise, inundation)
                    WITH slr AS (SELECT geometry as geom FROM slr_raw WHERE rise='{rise}' AND inundation='{inundation}')
                    SELECT id_orig, geoid_county, {rise}, '{inundation}'
                    FROM origins20_snap, slr
                    WHERE ST_Intersects(origins20_snap.geometry, slr.geom);
                    """.format(rise=rise, inundation=inundation)
                       ]
            logger.error(
                'Creating/Appending exposed origins_snap to SQL table | {}-{}'.format(rise, inundation))
            for q in queries:
                cur.execute(q)
            conn.commit()
...
post_message_to_slack(
    "Origins20_snap exposed to SLR have been determined and written to SQL")
