...
def calc_exposure(region_pair, config):
    ...
    tmp_exists = pd.read_sql("select exists(select * from information_schema.tables where table_name='tmp_{}_{}')".format(
        state.lower(), region.lower()), db['engine']).exists[0]
    if not tmp_exists:
        sql = """WITH slr AS (SELECT geometry as geom FROM slr_raw WHERE region='{region}' AND rise='{rise}')
                SELECT geoid, geoid_county, geometry, "H7X001", "H7X002", "H7X003", "H7Y003"
                FROM blocks, slr
                WHERE ST_Intersects(blocks.geometry, slr.geom)
                AND blocks."H7X001">0;
        ...
        footprint_blocks = gpd.read_file(
            './data/raw/footprint/{}.geojson.zip'.format(state), mask=blocks)
        # sjoin the block geoid to the footprint
        # footprint_blocks1 = gpd.sjoin(footprint_blocks, blocks, how='left')
        footprint_blocks = gpd.sjoin(
            footprint_blocks, blocks, how='left', op='within')
        ...
        block_areas = footprint_blocks[[
            'geoid', 'area']].groupby('geoid').sum()
        ...
        footprint_blocks = footprint_blocks[[
            'geometry', 'geoid', 'geoid_county']+demographics]
        blocks = blocks[['H7X001m2', 'H7X002m2',
                         'H7X003m2', 'H7Y003m2']].reset_index()
        footprint_blocks = footprint_blocks.merge(
            blocks, how='left', on='geoid')
    ...
    for rise in tqdm(np.arange(0, 11)):
        ...
        footprint_exposed = gpd.read_postgis(
            sql, con=db['engine'], geom_col='geometry', crs=crs)
        footprint_exposed.drop_duplicates(inplace=True)
        logger.info('Footprint {}, {}, {}ft imported'.format(
            region, state, rise))
        ...
        for i in demographics:
            footprint_exposed[i] = footprint_exposed['{}m2'.format(
                i)]*footprint_exposed['area']
        # sum to get population in this county, region
        population_exposed = footprint_exposed[[
            'geoid_county'] + demographics].groupby('geoid_county').sum()
        population_exposed.reset_index(inplace=True)
        # append to results
        if len(population_exposed) > 0:
            population_exposed = population_exposed[[
                'geoid_county', 'H7X001', 'H7X002', 'H7X003', 'H7Y003']]
    ...
    results['id'] = results['state'] + '_' + \
        results['region'] + results['rise'].astype(str)
    results.set_index('id', inplace=True)
    results.to_sql('exposed_people10', con=db['engine'], if_exists='append')
    # drop tmp table
    db['con'].cursor().execute(
        "DROP TABLE tmp_{}_{}".format(state.lower(), region.lower()))
    db['con'].commit()
    db['con'].close()
    db['engine'].dispose()
...
results.to_csv('./data/processed/exposed10.csv')

post_message_to_slack(
    "Estimate for SLR 2010 exposed people for the entire USA is complete")
