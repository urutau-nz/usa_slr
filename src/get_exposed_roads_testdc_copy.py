...
def main_road():
    ...
    for region in tqdm(regions):
        ...
        for rise in tqdm(rises):
            for inundation in tqdm(inundations):
                logger.error('Computing exposure for: {}-{}-{}'.format(region, rise, inundation))
                #conn = db['engine'].raw_connection()
                #cur = conn.cursor()
                cursor = db['con'].cursor()
                queries = ["SET work_mem = '500MB'", 
                        """ALTER TABLE slr_raw
                        ALTER COLUMN geometry
                        SET STORAGE EXTERNAL;""",
                        """ALTER TABLE temp_roads_test
                        ALTER COLUMN geometry
                        SET STORAGE EXTERNAL;""",
                        ...
                        """INSERT INTO exposed_roads (from_osmid, to_osmid, length, {rise}, {inundation}, {region})
                            SELECT from_osmid, to_osmid, length FROM temp_roads_test AS roads,
                            (SELECT geometry FROM slr_raw WHERE rise='{rise}' AND inundation='{inundation}' AND region='{region}') as slr 
...
queries = ["SET work_mem = '500MB'", 
        ...
        """CREATE TABLE IF NOT EXISTS exposed_roads 
            ...
            inundation VARCHAR(255),
            region VARCHAR(255));""",
        """EXPLAIN ANALYZE (SELECT from_osmid, to_osmid, length FROM temp_roads_test AS roads,
            (SELECT geometry FROM dc_test) as slr 
            WHERE ST_Intersects(roads.geometry, slr.geometry));
            """.format(rise=rise, inundation=inundation, region=region)
        ]
...
EXPLAIN ANALYZE WITH slr AS (SELECT
    st_subdivide(geometry) geometry
    FROM dc_test2)
SELECT
    ST_Intersects(slr.geometry, roads.geometry)
    FROM dc_test2 slr, temp_roads_test roads;
